/**
 * Place Insights Service - Mongo API
 *
 * Fetches per-place insights as DataBlocks with cache and safe fallback.
 */
...
const CACHE_KEY = 'ujuz.insights.cache.v2';
...
function normalizeBlock(block?: any): DataBlock | undefined {
  if (!block) return undefined;
  const updatedAt = block.updatedAt || block.updated_at;
  return {
    ...block,
...
function normalizeInsights(row: any): PlaceInsights {
  if (row.waitTime || row.crowdLevel || row.safetyScore || row.dealCount || row.peerVisits) {
    return {
      waitTime: normalizeBlock(row.waitTime),
      crowdLevel: normalizeBlock(row.crowdLevel),
      safetyScore: normalizeBlock(row.safetyScore),
      dealCount: normalizeBlock(row.dealCount),
      peerVisits: normalizeBlock(row.peerVisits),
    };
  }
...
export const insightsService = {
  async getInsightsForPlaces(placeIds: string[]): Promise<InsightsResult> {
    ...
    try {
      const result = await apiGet<any>('/insights', { placeIds: uniqueIds });
      const rows = Array.isArray(result) ? result : result.insights || result.data || [];

      const map = new Map<string, PlaceInsights>();
      rows.forEach((row: any) => {
        const id = row.place_id || row.placeId || row.id;
        if (!id) return;
        map.set(id, normalizeInsights(row));
      });

      // Merge with mock for missing ids
      uniqueIds.forEach((id) => {
        if (!map.has(id)) {
          const mock = MOCK_PLACE_INSIGHTS.get(id);
      ...
      const merged: Record<string, PlaceInsights> = {
        ...(cache?.data
          ? Object.fromEntries(
      ...
      return { insights: map, source: 'mongo' };
    } catch (error) {
      console.warn('[MongoInsights] Failed to fetch insights:', error);
      const fallback = new Map<string, PlaceInsights>();
      uniqueIds.forEach((id) => {
        const mock = MOCK_PLACE_INSIGHTS.get(id);
        if (mock) fallback.set(id, mock);
