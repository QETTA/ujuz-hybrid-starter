...
async function runJob(aiJobId: string) {
  const job = await prisma.aiJob.findUnique({ where: { id: aiJobId } });
  if (!job) return;

  await prisma.aiJob.update({ where: { id: aiJobId }, data: { status: 'RUNNING' } });
  ...
  try {
    ...
    await prisma.aiJob.update({ where: { id: aiJobId }, data: { status: 'SUCCEEDED', output } });
  } catch (e: any) {
    await prisma.aiJob.update({ where: { id: aiJobId }, data: { status: 'FAILED', error: String(e?.message ?? e) } });
  }
}
...
const worker = new Worker(
  ...
  async (bullJob) => {
    ...
    if (!aiJobId) return;
    await runJob(aiJobId);
  },
  { connection: { url: redisUrl } }
);

worker.on('completed', (job) => console.log(`✅ ai job completed: ${job.id}`));
